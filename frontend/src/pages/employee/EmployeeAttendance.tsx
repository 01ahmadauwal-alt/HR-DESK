...
export default function EmployeeAttendance() {
  ...
  const { data: records = [], isLoading } = useQuery<Attendance[]>({
    ...
    queryFn: () => api.get(`/employee/attendance?month=${month}&year=${year}`).then((r) => r.data),
  });
          ...
          {isLoading ? (
            ...
            <div className="table-wrapper">
              <table className="table">
                ...
                <tbody>
                  {records.length === 0 ? (
                    <tr><td colSpan={6} className="text-center py-8 text-slate-400 text-sm">No records for this period</td></tr>
                  ) : (
                    records.map(r => (
                      <tr key={r._id}>
