...
export function applyTaxBrackets(annualTaxable: number, brackets: ITaxBracket[]): number {
  let tax = 0;
...
export function calculatePayroll(
  ...
  loanRepayment: number,
  salaryAdvanceRecovery: number,
  overrides: IEmployeeDeductionOverride[]
): PayrollResult {
  ...
  const customDeductions: { name: string; amount: number }[] = [];
