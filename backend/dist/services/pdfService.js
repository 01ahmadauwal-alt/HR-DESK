...
async function generatePayslipPdf(payroll, employee, outputDir) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (payroll.nhf)
            deductionLines.push(['NHF (2.5% of Basic)', formatNaira(payroll.nhf)]);
        if (payroll.lifeInsurance)
            deductionLines.push(['Life Insurance', formatNaira(payroll.lifeInsurance)]);
        if (payroll.healthInsurance)
            deductionLines.push(['Health Insurance (HMO)', formatNaira(payroll.healthInsurance)]);
        if (payroll.unionDue)
            deductionLines.push(['Union Dues', formatNaira(payroll.unionDue)]);
        if (payroll.cooperative)
            deductionLines.push(['Cooperative', formatNaira(payroll.cooperative)]);
        if (payroll.staffWelfareLevy)
            deductionLines.push(['Staff Welfare Levy', formatNaira(payroll.staffWelfareLevy)]);
        if (payroll.loanRepayment)
            deductionLines.push(['Loan Repayment', formatNaira(payroll.loanRepayment)]);
        if (payroll.salaryAdvanceRecovery)
            deductionLines.push(['Salary Advance Recovery', formatNaira(payroll.salaryAdvanceRecovery)]);
        ...
        y = drawTable(doc, y, 'EMPLOYER CONTRIBUTIONS', [
            ...
            ...(payroll.groupLifeAssurance ? [['Group Life Assurance', formatNaira(payroll.groupLifeAssurance)]] : []),
        ]);
        y += 20;
        // Net pay box
...
function drawTable(doc, startY, heading, rows, totalValue, totalLabel) {
    const left = 50;
